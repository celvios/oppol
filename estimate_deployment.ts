...
async function main() {
    ...
    // We need a signer to estimate deployment transaction accurately? 
    // Usually provider.estimateGas({ data: bytecode }) works.

    // Note: If the contract has a constructor arguments, we must encode them.
    // V3 constructor is empty/disabled Initializers?
    // constructor() { _disableInitializers(); } 
    // So no arguments needed for deployment transaction itself.
