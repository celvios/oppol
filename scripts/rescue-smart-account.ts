/**
 * rescue-smart-account.ts
 * 
 * 0xd0A115... is an undeployed Biconomy (Nexus/Simple) Smart Account.
 * Because it hasn't sent a transaction yet, it has no code (looks like an EOA).
 * We will use permissionless.js to send a UserOperation signed by the owner's private key.
 * This will automatically deploy the contract and transfer the USDC.
 */
...
async function main() {
    ...
    if (simpleAccount.address.toLowerCase() !== '0xd0a115ea64b59f951b70276fcb65b4946465e3a9') {
        ...
        // Note: we can upgrade this script to try different account types if this fails.
    }
